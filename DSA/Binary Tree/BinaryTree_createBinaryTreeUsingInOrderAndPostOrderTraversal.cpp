...
// so here we are taking all the array postOrderIndex inOrderStart inOrderEnd n and mapping in the input
// so the use of the postOrderIndex is that it stores till what we have reached in the postOrderTraversal
...
Node* solve(int inOrderTraversal[],int postOrderTraversal[],int &postOrderIndex,int inOrderStart,int inOrderEnd,int n,map<int,int>& nodeToIndex){
    if(postOrderIndex<0 || inOrderStart>inOrderEnd){ // if the postOrderIndex is less than 0 or if the inOrderStart becomes more than inOrrder end
        return NULL; // then return NULL cause no node will present at that position
    }

    int element = postOrderTraversal[postOrderIndex--]; // now we have to find the element for the node
    ...
    root->right = solve(inOrderTraversal,postOrderTraversal,postOrderIndex,position+1,inOrderEnd,n,nodeToIndex); // we will be going in the right of the inOrderTraversal to find the right node of the element
    root->left = solve(inOrderTraversal,postOrderTraversal,postOrderIndex,inOrderStart,position-1,n,nodeToIndex); // we will be going in the left of the inOrderTraversal to find the left node of the element
    
    return root; // return root when these things are done
}

Node* buildTree(int inOrderTraversal[],int postOrderTraversal[],int n){
    ...
    Node* ans = solve(inOrderTraversal,postOrderTraversal,postOrderIndex,0,n-1,n,nodeToIndex); // making a answer node and calling the solve function
