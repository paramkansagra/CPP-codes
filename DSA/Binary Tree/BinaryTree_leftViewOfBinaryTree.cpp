...
void leftViewOfTree(Node* root,vector<int>& ans,int level){
    ...
    leftViewOfTree(root->left,ans,level+1); // go in the left side of the tree
    leftViewOfTree(root->right,ans,level+1); // go in the right side of the tree
}

void print(Node* root){ // bring the root node for the printing
    vector<int> ans; // pull the answer vector from the leftViewOfTree
    leftViewOfTree(root,ans,0);
